...
- **Notifikasi HP** ada di `services/push_notification_service.dart` (`flutter_local_notifications`). Backend belum punya FCM, jadi `NotificationCenter.syncFromServer()` yang ditarik `MainScreen` tiap 2 menit + saat app resume-lah yang memunculkan notifikasi backend; peringatan yang dibangkitkan app (batas lembur, lupa check-out) lewat `NotificationCenter.alertOnce`. Setiap teks yang keluar dilewatkan `services/notification_menu_hints.dart` supaya menyebut menu tujuan ("Buka menu Akun > Dokumen Saya ..."); tambahkan `type` baru ke peta di file itu, bukan menempel nama menu di teks backend.
